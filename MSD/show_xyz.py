...
def go(file, show_errorbars=False, show_fit=False, SHOW_SHORT_FIT=True, SHOW_LONG_FIT=False, export_destination=None):
    data = common.load(f'MSD/data/msd_xyz_{file}.npz')
    all_msd        = data['msd']
    ...
    for dimension in range(num_dimensions):
        ...
        ax.plot(t[1:], msd[1:], marker='.', markersize=8, linestyle='none', color=color, label='xyz'[dimension])
        if show_errorbars:
