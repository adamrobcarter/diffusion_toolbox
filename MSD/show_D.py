...
def go(file, ax, index_exp=np.index_exp[:], color=None, markersize=3):
    ...
    D_unc = D * msd_unc/msd
    ax.plot(t[index_exp], D[index_exp], marker='.', markersize=markersize, linestyle=r'none',
            label=r'$1/4 \cdot \mathrm{d}\langle r^2 \rangle/\mathrm{d}t$',
            color=color)
    ...
    ax.set_ylabel(r'$D_\mathrm{self}$ (\si{\micro\meter\squared / \second})')
    ax.set_xlabel('$t$ (s)')
    # ax.legend()
...
if __name__ == '__main__':
    for file in common.files_from_argv('MSD/data', 'msd_'):
        ...
        go(file, ax, index_exp=np.index_exp[:-200])
