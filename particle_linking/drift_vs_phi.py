...
def go(files, ax, eleanor=False):
    ...
    if len(files):
        ax.scatter(phis, drifts/np.sin(10*np.pi/180), label='simulation')
    else:
    ...
    if eleanor:
        ...
        ax.scatter(eleanor_phis, eleanor_vs, label='experiment')
...
if __name__ == '__main__':
    ...
    go(files, ax)
    common.save_fig(fig, f'particle_linking/figures_png/drift_vs_phi_{files[0]}.png')