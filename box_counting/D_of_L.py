...
for file in sys.argv[1:]:
    ...
    for box_size_index in tqdm.trange(num_boxes):
        ...
        if True:
            ...
            line = integrand_ax.plot(t[1:M2_index], T_integrand[1:M2_index], label=f'L={L:.2f}', zorder=5)
            integrand_ax.plot(t[M2_index:], T_integrand[M2_index:], alpha=0.5, color=line[0].get_color(), zorder=4)
            ...
            full_theory_N2 = lambda t, D0 : sDFT_interactions.sDFT_interactions(L=L, t=t, phi=phi, D0=D0, sigma=sigma) * 2 * N_mean[box_size_index]
            #                                                                                                          ^^^ these bits actually do nothing cause we take the ratio later
            ...
            integrand_ax.plot(t_fit_late, late_fit_func_real(t_fit_late, *late_popt), color='grey', linewidth=1, label='counto. SI fit' if box_size_index==0 else None, zorder=6)
