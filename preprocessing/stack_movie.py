...
for file in sys.argv[1:]:
    ...
    stack_rm = stack - stack.mean(axis=0) # remove space background
...
# 
    frames = range(0, min(stack.shape[0], 1000), 10)

    def show(timestep):
        ax.clear()
        # plt.imshow(stack[timestep, :, :])
        # plt.imshow(stack[timestep, :, :])
        im = ax.imshow(stack_rm[timestep, :, :], vmin=stack.min(), vmax=stack.max())
        if timestep == 0:
            fig.colorbar(im)
